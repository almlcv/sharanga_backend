...
# FastAPI Router
router = APIRouter(tags=["PES COVER - B LH-RH positioning detection"])

# Load Models
detect_model = YOLO("models/pes-b-det.pt")
classify_model = YOLO("models/pes-b-cls.pt")
...
# @router.post("/upload-pes-cover-B-classification", response_model=dict)
# def upload_pes_cover_B_image(expected_type: str, file: UploadFile = File(...), current_user: User = Depends(production_required)):
...
# @router.get("/pes-cover-B-images-data-classification", response_model=dict)
# def get_all_pes_cover_B_user_images(current_user: User = Depends(production_required)):
#     """
...
# @router.delete("/delete-pes-cover-B-classification/{file_id}", response_model=dict)
# def delete_pes_cover_B_uploaded_file(file_id: str, current_user: User = Depends(production_required)):
#     """
...
# ======================================================
# 🧠 Upload and Process Pes Cover B Image
# ======================================================
@router.post("/upload-pes-cover-B-classification", response_model=dict)
def upload_pes_cover_B_image(
    expected_type: str,
    ...
    try:
        ...
        logger.info(f"✅ Saved PesCoverB classification data to MongoDB for {current_user.name}")
...
def get_all_pes_cover_B_user_images(current_user: User = Depends(production_required)):
    try:
        # user_name = current_user.name
        part_name = "PesCoverB"
        # Find all user-specific PesCoverB records
        records = list(
        ...
        logger.error(f"Error fetching PesCoverB records: {e}")
        raise HTTPException(status_code=500, detail=str(e))
