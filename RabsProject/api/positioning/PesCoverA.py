...
# FastAPI Router
router = APIRouter(tags=["PES COVER - A LH-RH positioning detection"])

# Load Models
detect_model = YOLO("models/pes-a-det.pt")
classify_model = YOLO("models/pes-a-cls.pt")
...
# @router.post("/upload-pes-cover-A-classification", response_model=dict)
# def upload_pes_cover_A_image(expected_type: str, file: UploadFile = File(...), current_user: User = Depends(production_required)):
...
# @router.get("/pes-cover-A-images-data-classification", response_model=dict)
# def get_all_pes_cover_A_user_images(current_user: User = Depends(production_required)):
#     """
...
# @router.delete("/delete-pes-cover-A-classification/{file_id}", response_model=dict)
# def delete_pes_cover_A_uploaded_file(file_id: str, current_user: User = Depends(production_required)):
#     """
...
# ======================================================
# 🧠 Upload and Process Pes Cover A Image
# ======================================================
@router.post("/upload-pes-cover-A-classification", response_model=dict)
def upload_pes_cover_A_image(
    expected_type: str,
    ...
    try:
        ...
        logger.info(f"✅ Saved PesCoverA classification data to MongoDB for {current_user.name}")
...
def get_all_pes_cover_A_user_images(current_user: User = Depends(production_required)):
    try:
        # user_name = current_user.name
        part_name = "PesCoverA"
        # Find all user-specific PesCoverA records
        records = list(
        ...
        logger.error(f"Error fetching PesCoverA records: {e}")
        raise HTTPException(status_code=500, detail=str(e))
